...
| 2 | `symbol_pua` | md | Remaps Private Use Area codepoints that publisher PDFs emit for Symbol-font and Wingdings-font glyphs (π, Σ, →, ≠, ∇, ×, ✓). Every entry in its table was verified against a rendered page. Each entry records the book, the PDF page and the font that emitted the codepoint. Also turns a PUA bullet marker at line start into a real list item. A Symbol-font space becomes a real space, but one at a line edge is dropped, because two of them are a hard break in Markdown. Scoped **outside** code fences. Unrecognized PUA is left alone and reported. A bullet marker that opens a line is also left alone and reported, if it cannot be read as a list item. That occurs when it is indented four spaces or more, or has no space after it. Deletion would flatten a nested list. A line-opening `U+F0B7` is also left alone, because it is a multiplication dot in one book and a list bullet in another. Runs a second time after `caption_unbleed`, which can promote a glyph from code to prose when it unwraps a fence. | md with real characters instead of invisible ones |
