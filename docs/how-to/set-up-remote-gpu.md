...
- A mid-run SSH drop does not necessarily kill a detached remote job. On reconnect, check before
  you assume the work was lost. pdf2wiki sends SSH keepalives on every remote call to hold the control
  channel open across long silent conversion passes. If a drop still happens, a re-run resumes from
  the cached (`.done`) passes. See [remote convert marked failed but the box continued](troubleshoot.md#remote-convert-marked-failed-but-the-box-continued).
